...
| **P3** action stages | Cascade + Vanguard Bounty (multi-placement) | median placements/turn >1 · SkillDepth into 65-85% · ply/balance held | ⚙ BUILT + MEASURED, verdict AWAITING OPERATOR. Best R8 result: fun 39.1→41.9 CASCADE-only (+2.8; 42.7 at density 4), K .541→.633, captures/balance/comeback/ply all held, no exploit (cascrush LOSES 25-32%), claims symmetric, UI browser-verified. 4 gate letters fail as mis-specified: median=1 (structural below ~50% density; multi-turn share 14-18%), SkillDepth 93.8-95.8% (not 65-85), cascrush under-band, firstCap T9 (standing debt). Recommendation: ship both ON. RULES8_P3_CASCADE.md status log has the full package. |
